...
if $(aws cloudformation list-stacks --stack-status-filter CREATE_COMPLETE | grep -q 'test-sagemaker-cicd') || $(aws cloudformation list-stacks --stack-status-filter UPDATE_COMPLETE | grep -q 'test-sagemaker-cicd' | head); then
    echo 'Updating Stack...'
